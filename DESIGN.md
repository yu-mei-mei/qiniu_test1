...
系统将语音识别和绘图决策解耦：ASR 只负责把声音转写成文本，后端再根据文本意图决定走 Canvas 指令绘图还是 AI 图片生成。

| 路由 | 适用指令 | 原因 |
|------|----------|------|
| Canvas 绘图 | 圆形、矩形、三角形、线条、点、文字、颜色、位置、清空、撤销、重做等基础操作 | 可参数化、低延迟、可控性强 |
| Canvas 绘图 | 简单组合对象，如房子、太阳、树等 | 能拆解为少量基础图形，适合结构化执行 |
| AI 生图 | 熊大、熊二、哆啦A梦/哆唻A梦、机器猫、蓝胖子、人物、动物、机器人等角色或对象 | 视觉质量依赖整体形象，Canvas 几何拼图效果差 |
...
后端响应会返回 `draw_mode` 和 `route_reason`，前端指令历史同步显示“Canvas 绘图 / AI 生图 / Canvas 回退”，便于演示和评审确认当前执行路径。
