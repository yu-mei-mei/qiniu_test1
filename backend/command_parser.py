...
### 绝对位置
- "中间"、"中心" → (400, 300)
- "左上角" → (50, 50)， "右上角" → (750, 50)
- "左下角" → (50, 550)， "右下角" → (750, 550)
- "左边" → x=100-200, y=300; "右边" → x=600-700, y=300
- "顶部" → y=50-100; "底部" → y=500-550
- "正上方"、"正下方" → 与参照物同x，y偏移

### 相对位置（参照已有对象）
- "A的左边" → A.x - (A宽度/2 + 间距)，同y
- "A的右边" → A.x + (A宽度/2 + 间距)，同y
- "A的上方" → 同x，A.y - (A高度/2 + 间距)
- "A的下方" → 同x，A.y + (A高度/2 + 间距)
- "在A里面"、"在A内部" → 坐标在A的范围内，尺寸小于A
- "A和B之间" → 取A和B的中点
- "围绕A" → 以A为中心均匀分布

### 复合布局
- "排成一行" → 从左到右均匀分布，间隔80-120px
- "排成一列" → 从上到下均匀分布，间隔80-120px
- "在四个角" → (50,50), (750,50), (50,550), (750,550)
- "对称" → 以中心线为轴对称
- "等距"、"均匀分布" → 计算间距平均分配

### 尺寸推断
- 大圆/大号 → radius=120-150; 中圆/中号 → radius=70-90; 小圆/小号 → radius=30-50
- 大矩形 → 300x300+; 中矩形 → 150x150; 小矩形 → 60x60
- "大一点"、"小一点" → 相对上一个对象尺寸 ±30%

### 多对象布局
- 当指令包含多个图形时，自动布局避免重叠
- 如果用户说"画一个房子"等复合对象，拆解为多个基本图形
- "左边画X，右边画Y" → X在左半区(100-350)，Y在右半区(450-700)
